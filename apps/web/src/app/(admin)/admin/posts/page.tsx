  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/posts/new" className={buttonVariants({ size: 'sm' })}><Plus className="size-4" />New Post</Link>
      </div>
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 'date', header: 'Created', cell: (p) => <span className="text-xs text-muted-foreground">{new Date(p.createdAt).toLocaleDateString()}</span> },
          {
            ...
            cell: (p) => (
              <Link href={`/admin/posts/${p.id}/edit`} className={buttonVariants({ variant: 'ghost', size: 'icon-sm' })}><Pencil className="size-3.5" /></Link>
            ),
          },
        ]}
